...
class group extends DbTable
{
	...
	public function __construct()
	{
		...
		// id
		$this->id = new DbField('group', 'group', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// name
		$this->name = new DbField('group', 'group', 'x_name', 'name', '`name`', '`name`', 200, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->name->Sortable = TRUE; // Allow sort
		$this->fields['name'] = &$this->name;

		// role
		$this->role = new DbField('group', 'group', 'x_role', 'role', '`role`', '`role`', 200, -1, FALSE, '`role`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`group`";
	}
	...
	{
		global $Language;
		if ($pageName == "groupview.php")
			return $Language->Phrase("View");
		elseif ($pageName == "groupedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "groupadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("groupview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("groupview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "groupadd.php?" . $this->getUrlParm($parm);
		else
			$url = "groupadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("groupedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("groupadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("groupdelete.php", $this->getUrlParm());
	}
