...
class product extends DbTable
{
	...
	public function __construct()
	{
		...
		// product_code
		$this->product_code = new DbField('product', 'product', 'x_product_code', 'product_code', '`product_code`', '`product_code`', 200, -1, FALSE, '`product_code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->product_code->IsPrimaryKey = TRUE; // Primary key field
		...
		// product_name
		$this->product_name = new DbField('product', 'product', 'x_product_name', 'product_name', '`product_name`', '`product_name`', 200, -1, FALSE, '`product_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->product_name->Sortable = TRUE; // Allow sort
		$this->fields['product_name'] = &$this->product_name;

		// product_price
		$this->product_price = new DbField('product', 'product', 'x_product_price', 'product_price', '`product_price`', '`product_price`', 3, -1, FALSE, '`product_price`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('product_code', $rs))
				AddFilter($where, QuotedName('product_code', $this->Dbid) . '=' . QuotedValue($rs['product_code'], $this->product_code->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('product_code', $row) ? $row['product_code'] : NULL) : $this->product_code->CurrentValue;
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@product_code@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getViewUrl($parm = "")
	{
		if ($parm <> "")
			...
			$url = $this->keyUrl("productview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		$json = "";
		$json .= "product_code:" . JsonEncode($this->product_code->CurrentValue, "string");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm <> "")
			$url .= $parm . "&";
		if ($this->product_code->CurrentValue != NULL) {
			$url .= "product_code=" . urlencode($this->product_code->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("product_code") !== NULL)
				$arKeys[] = Param("product_code");
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->product_code->CurrentValue = $key;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					if ($this->product_code->Exportable)
						$doc->exportCaption($this->product_code);
					if ($this->product_name->Exportable)
						$doc->exportCaption($this->product_name);
					if ($this->product_price->Exportable)
