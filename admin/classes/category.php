...
class category extends DbTable
{
	...
	public function __construct()
	{
		...
		// category_id
		$this->category_id = new DbField('category', 'category', 'x_category_id', 'category_id', '`category_id`', '`category_id`', 3, -1, FALSE, '`category_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// name
		$this->name = new DbField('category', 'category', 'x_name', 'name', '`name`', '`name`', 200, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->name->Sortable = TRUE; // Allow sort
		$this->fields['name'] = &$this->name;

		// icon
		$this->icon = new DbField('category', 'category', 'x_icon', 'icon', '`icon`', '`icon`', 200, -1, FALSE, '`icon`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->icon->Sortable = TRUE; // Allow sort
		$this->fields['icon'] = &$this->icon;

		// status
		$this->status = new DbField('category', 'category', 'x_status', 'status', '`status`', '`status`', 16, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		global $Language;
		if ($pageName == "categoryview.php")
			return $Language->Phrase("View");
		elseif ($pageName == "categoryedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "categoryadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("categoryview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("categoryview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "categoryadd.php?" . $this->getUrlParm($parm);
		else
			$url = "categoryadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("categorydelete.php", $this->getUrlParm());
	}
